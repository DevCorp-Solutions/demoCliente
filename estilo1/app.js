/**
 * DevCorp GastroSuite - Módulo Autónomo para ESTILO1
...
const STORAGE_KEY = 'devcorp_gastrosuite_state_estilo1_v1';
...
class GastroApp {
  ...
  renderMenuView(container, preset) {
    ...
    const layoutHtml = this.renderEstilo1Layout(preset, filteredMenu, activeCategory, activeAllergens, topData);;
    ...
    return `
      ...
      <!-- SECCIÓN DE RESERVAS FRENTE A LAS BRASAS (Fondo alargado de extremo a extremo, estilo idéntico a opiniones) -->
  ...
  // -------------------------------------------------------------------------
    ...
    const selectorHtml = `
      <select id="style-select" class="text-xs font-semibold rounded-xl py-2 px-3 focus:outline-none focus:ring-2 transition-all cursor-pointer shadow-sm ${
        ...
        <option value="estilo2" ${preset.id === 'estilo2' ? 'selected' : ''}>Estilo 2: Punto de Encuentro</option>
