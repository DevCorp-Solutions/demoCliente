...
const cartaPath = path.join(__dirname, '..', 'Punto de Encuentro - Sitio web del bar Estilo4', 'Carta.dc.html');
const content = fs.readFileSync(cartaPath, 'utf8');
...
const match = content.match(/C\s*=\s*(\[[\s\S]*?\n\s*\]);/);
...
categories.forEach(([catId, catEs, catEn, note, items]) => {
  categoryNames.push(catEs);
  items.forEach(it => {
    const rawName = it[0];
    const enName = it[1];
    const rawPrice = it[2];
    const allergenCodes = it[3] ? it[3].split(' ').filter(Boolean) : [];
    const desc = it[4] || enName || '';
    const numPrice = parseFloat(rawPrice.replace(',', '.').replace(/[^\d.]/g, '')) || 0;
    ...
    menu.push({
      ...
      nameEn: enName,
      price: numPrice,
      priceFormatted: rawPrice.includes('€') ? rawPrice : rawPrice + ' €',
      category: catEs,
      categoryId: catId,
      description: desc,
      allergens: allergenCodes.map(c => AL[c] || c),
...
const outputJs = `// estilo2/data.js - Datos oficiales y configurables para Bar Punto de Encuentro
(function() {
...
const destPath = path.join(__dirname, '..', 'estilo2', 'data.js');
fs.writeFileSync(destPath, outputJs, 'utf8');
console.log('Successfully wrote estilo2/data.js, total dishes:', menu.length);
