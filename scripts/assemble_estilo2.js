...
puntoSrc = puntoSrc.replace(oldHeadScript, newHeadScript);
...
const newHeaderHtml = `<header data-screen-label="Portada" style="position:relative;background-color:#EBF4F3;background-image:url('data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%27220%27 height=%27220%27%3E%3Cfilter id=%27n%27%3E%3CfeTurbulence type=%27fractalNoise%27 baseFrequency=%27.85%27 numOctaves=%273%27/%3E%3CfeColorMatrix values=%270 0 0 0 0.13 0 0 0 0 0.23 0 0 0 0 0.24 0 0 0 0 0.10 0%27/%3E%3C/filter%3E%3Crect width=%27100%25%27 height=%27100%25%27 filter=%27url(%23n)%27/%3E%3C/svg%3E'),url('data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%27900%27 height=%27900%27%3E%3Cfilter id=%27s%27%3E%3CfeTurbulence type=%27fractalNoise%27 baseFrequency=%27.006%27 numOctaves=%274%27 seed=%277%27/%3E%3CfeColorMatrix values=%270 0 0 0 0.25 0 0 0 0 0.43 0 0 0 0 0.45 0 0 0 0.16 0%27/%3E%3C/filter%3E%3Crect width=%27100%25%27 height=%27100%25%27 filter=%27url(%23s)%27/%3E%3C/svg%3E');overflow:hidden">
  ...
  <!-- CABECERA DEVCORP ADAPTADA AL ESTILO DEL LOCAL (AMPLIADA Y CENTRADA) -->
  <div style="padding:16px clamp(14px,3vw,36px) 0;width:100%;box-sizing:border-box">
    <div style="max-width:${MAX_WIDTH};margin:0 auto;background:#EBF4F3;border:2px solid #213A3C;border-radius:6px;box-shadow:3px 3px 0 #213A3C;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px 24px;padding:12px 22px;font-family:'Alegreya Sans SC',sans-serif;font-weight:800;font-size:14px;letter-spacing:.1em">
      ...
      <div style="display:flex;align-items:center;gap:14px">
        ...
        <div style="display:flex;flex-direction:column;line-height:1.15">
          ...
          <span style="font-family:'Alegreya Sans SC',sans-serif;font-weight:800;font-size:11px;letter-spacing:0.12em;color:#3F6E72">Bar Cafetería · Aluche</span>
        </div>
      </div>

      <!-- Centro: Enlaces de Navegación del Local (Sin Reservas ni Cocina/KDS) -->
      <nav aria-label="Navegación principal" style="display:flex;flex-wrap:wrap;gap:4px 20px;align-items:center">
...
puntoModified = puntoModified.replace(
  ...
  `<div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap;align-items:stretch">`
);

// Centering Línea del día
puntoModified = puntoModified.replace(
  '<div style="padding:34px clamp(18px,4vw,56px) 40px;border-top:2px solid #213A3C">',
  `<div style="padding:34px clamp(18px,4vw,56px) 40px;border-top:2px solid #213A3C"><div style="max-width:${MAX_WIDTH};margin:0 auto">`
);
...
puntoModified = puntoModified.replace(
  ...
  `$1<div style="max-width:${MAX_WIDTH};margin:0 auto">`
);
...
puntoModified = puntoModified.replace(
  ...
  `<div style="padding:clamp(56px,8vw,120px) clamp(18px,4vw,56px) 0"><div style="max-width:${MAX_WIDTH};margin:0 auto">`
);
puntoModified = puntoModified.replace(
...
puntoModified = puntoModified.replace(
  ...
  `<div style="margin:clamp(40px,6vw,80px) 0 0;background:#213A3C;color:#EBF4F3;padding:clamp(28px,4vw,56px) clamp(18px,4vw,56px);position:relative"><div style="max-width:${MAX_WIDTH};margin:0 auto">`
);
puntoModified = puntoModified.replace(
  '<!-- Promos y raciones -->',
  '</div></div>\n  <!-- Promos y raciones -->'
...
puntoModified = puntoModified.replace(
  ...
  `<div style="padding:clamp(40px,6vw,80px) clamp(18px,4vw,56px) clamp(56px,7vw,100px)"><div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap;gap:clamp(34px,5vw,80px)">`
);
puntoModified = puntoModified.replace(
...
puntoModified = puntoModified.replace(
  ...
  `<div style="background:#EBF4F3;border-top:2px solid #213A3C;padding:clamp(56px,8vw,110px) clamp(18px,4vw,56px)"><div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap;gap:clamp(30px,5vw,70px)">`
);
puntoModified = puntoModified.replace(
  '</section>\n\n<!-- ======== 23:00 FINDE ======== -->',
  '</div></div></section>\n\n<!-- ======== 23:00 FINDE ======== -->\n'
...
puntoModified = puntoModified.replace(
  /(<section data-screen-label="Opiniones"[^>]*>)/,
  `$1<div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap;gap:clamp(30px,5vw,70px);align-items:flex-start">`
);
puntoModified = puntoModified.replace(
  '</section>\n\n<!-- ======== CÓMO LLEGAR ======== -->',
  '</div></section>\n\n<!-- ======== CÓMO LLEGAR ======== -->\n'
...
puntoModified = puntoModified.replace(
  /(<section id="llegar"[^>]*>)/,
  `$1<div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap">`
);
puntoModified = puntoModified.replace(
  '</section>\n\n<footer',
  '</div></section>\n\n<footer'
...
puntoModified = puntoModified.replace(
  /(<footer[^>]*>)/,
  `$1<div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;flex-wrap:wrap;gap:18px 40px;justify-content:space-between;align-items:flex-end">`
);
puntoModified = puntoModified.replace(
  '</footer>\n\n<sc-if value="{{ narrow }}"',
  '</div></footer>\n\n<sc-if value="{{ narrow }}"'
...
console.log('Successfully wrote estilo2/index.html & Punto de Encuentro.dc.html with max-width: ' + MAX_WIDTH);
...
const newCartaHeaderHtml = `<header data-screen-label="Carta · cabecera" style="background:#213A3C;color:#EBF4F3">
  ...
  <!-- CABECERA DEVCORP ADAPTADA AL ESTILO DEL LOCAL (AMPLIADA Y CENTRADA) -->
  <div style="padding:16px clamp(14px,3vw,36px) 0;width:100%;box-sizing:border-box">
    <div style="max-width:${MAX_WIDTH};margin:0 auto;background:#183234;border:2px solid #3F6E72;border-radius:6px;box-shadow:3px 3px 0 #3F6E72;color:#EBF4F3;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px 24px;padding:12px 22px;font-family:'Alegreya Sans SC',sans-serif;font-weight:800;font-size:14px;letter-spacing:.1em">
  ...
  <!-- Título Original de la Carta y Estado (Centrado y Ampliado) -->
  <div style="max-width:${MAX_WIDTH};margin:0 auto;padding:18px clamp(16px,4vw,36px) 22px;display:flex;flex-wrap:wrap;align-items:flex-end;justify-content:space-between;gap:12px 24px">
    <div>
...
// Centering sticky category bar in Carta to MAX_WIDTH
cartaModified = cartaModified.replace(
  ...
  `<nav aria-label="Categorías" style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;gap:6px;overflow-x:auto;padding:10px clamp(16px,4vw,36px);scrollbar-width:none" ref="{{ navRef }}">`
);
cartaModified = cartaModified.replace(
  ...
  `<div style="max-width:${MAX_WIDTH};margin:0 auto;display:flex;gap:6px;overflow-x:auto;padding:0 clamp(16px,4vw,36px) 10px;align-items:center;scrollbar-width:none">`
);

// Expanding <main> in Carta from 1180px to MAX_WIDTH
cartaModified = cartaModified.replace(
  '<main style="max-width:1180px;margin:0 auto;padding:10px clamp(16px,4vw,48px) 40px">',
  `<main style="max-width:${MAX_WIDTH};margin:0 auto;padding:10px clamp(16px,4vw,36px) 40px">`
);
...
console.log('Successfully wrote estilo2/carta.html & Carta.dc.html with max-width: ' + MAX_WIDTH);
